...
trait LoginTrait
{
    ...
    public function handleSocialLogin(string $slug, PluginConfiguration $config, UserData $userData, array $userProfileData): void
    {
        ...
        if (empty($userId))
        {
            ...
            try
            {
                ...
                $userId = $this->createUser($email, $userData->name, $bypassVerification, $userData->timezone, $config);
                /**
     ...
     */
    private function createUser($email, $name, $emailVerified, $timezone, $config)
    {
        ...
        // Check if email follows the specific format: ATXXXXXX@actvn.edu.vn
        if (preg_match('/^([A-Z]{2}\d{6})@actvn\.edu\.vn$/', $email, $matches) & $config->linkedAccountUserName === true )
        {
            $username = $matches[1];
        }
        else
        ...
        return $this->register($config, $data, $userParams, $emailVerified);
    }
     ...
     * @throws  Exception
     */
    private function loginUser($userId, $config)
    {
        ...
        $user    = Factory::getContainer()->get(UserFactoryInterface::class)->loadUserById($userId);
        if (preg_match('/^[A-Z]{2}\d{6}@actvn\.edu\.vn$/', $user->email) === 0 && $config->blockNonAcademyAccounts === true) {
            throw new RuntimeException(Text::_('JGLOBAL_AUTH_ACCESS_DENIED_ACCOUNT_NOT_ACTVN'));
        }
     ...
     * @throws  Exception
     */
    private function register($config, array $data, array $userParams = [], $skipUserActivation = false)
    {
        $cParams = ComponentHelper::getParams('com_users');
        if (preg_match('/^[A-Z]{2}\d{6}@actvn\.edu\.vn$/', $data['email1']) & $config->linkedAccountUserName === true)
        {
            $data = array_merge([
                'name'      => '',
                'username'  => '',
                ...
                'email1'    => '',
                'groups'    => [$cParams->get('new_usertype', 2), $config->linkedUserGroupStudent],
                'params'    => $userParams,
            ], $data);
        }
        else if ($config->blockNonAcademyAccounts === false)
        {
        ...
        } else if ($config->blockNonAcademyAccounts === true) {
            throw new RuntimeException(Text::_('JGLOBAL_AUTH_ACCESS_DENIED_ACCOUNT_NOT_ACTVN'));
        }
